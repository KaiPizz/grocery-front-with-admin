...
const OWNER_REVIEW_PATH = 'docs/asiandeligo-folder14-draft-owner-review-20260713.csv';
...
const EXPECTED_UPDATE_ROWS = 47;
const EXPECTED_HOLD_ROWS = 6;
...
function loadPlan() {
  ...
  const rows = decisions.map((decision) => {
    ...
    if (!['update_content', 'hold_for_owner'].includes(decision.plan_action)) {
  ...
  const holds = rows.filter((row) => row.plan_action === 'hold_for_owner');
  if (updates.length !== EXPECTED_UPDATE_ROWS || holds.length !== EXPECTED_HOLD_ROWS) {
    throw new Error(`Expected ${EXPECTED_UPDATE_ROWS} updates and ${EXPECTED_HOLD_ROWS} holds; got ${updates.length}/${holds.length}`);
  }
  ...
  return { rows, updates, holds, extras };
}
...
function writeOutputs(plan) {
  const { rows, updates, holds, extras } = plan;
  ...
  const ownerReviewRows = holds.map((row) => ({
    review_id: row.review_id,
    draft_sku: row.draft_sku,
    proposed_name: row.proposed_pl_name,
    quality_flags: row.quality_flags,
    question: row.review_note,
    primary_url: row.primary_url,
  }));

  writeCsv(OWNER_REVIEW_PATH, ownerReviewRows, [
    'review_id', 'draft_sku', 'proposed_name', 'quality_flags', 'question', 'primary_url',
  ]);
  ...
  const summary = {
    ...
    owner_confirmation_rows: holds.length,
  ...
  const report = [
    ...
    `- Rows held for owner confirmation: ${holds.length}`,
    ...
    '| --- | --- | --- |',
    ...holds.map((row) => tableRow([row.draft_sku, row.proposed_pl_name, row.review_note])),
    '',
    ...
    '## Generated Artifacts',
    '',
    `- Owner review queue: \`${OWNER_REVIEW_PATH}\``,
