...
const REVIEW_PATH = 'docs/asiandeligo-catalog-metadata-batch5-reviewed-actions-20260722.json';
const OUTPUT_PATH = 'docs/asiandeligo-catalog-metadata-batch5-decisions-20260722.json';
...
function validateReview(review) {
  if (
    ...
    throw new Error('Reviewed actions do not match the exact batch-5 identity and cohort');
  }
...
async function build() {
  ...
  if (bySku.size !== 25 || COHORT_SKUS.some((sku) => !bySku.has(sku))) {
    throw new Error('Production snapshot does not match the exact batch-5 cohort');
  }
