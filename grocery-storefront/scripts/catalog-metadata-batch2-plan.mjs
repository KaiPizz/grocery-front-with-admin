...
const DEFAULT_INPUT = 'docs/asiandeligo-catalog-metadata-batch2-decisions-20260722.json';
const DEFAULT_OUTPUT = 'docs/asiandeligo-catalog-metadata-batch2-remediation-20260722.md';
const DEFAULT_APPLY = 'docs/asiandeligo-catalog-metadata-batch2-apply-20260722.sql';
const DEFAULT_ROLLBACK = 'docs/asiandeligo-catalog-metadata-batch2-rollback-20260722.sql';

const EXPECTED_BATCH = 'asiandeligo-catalog-metadata-batch2-20260722-v1';
const EXPECTED_CHANNEL = 'asiandeligo';
const EXPECTED_SALON_ID = 'e73271a9-53e3-4a20-a02e-791726b452aa';
const PRODUCT_BACKUP_TABLE = 'asiandeligo_catalog_metadata_batch2_product_backup_20260722';
const AUDIT_TABLE = 'asiandeligo_catalog_metadata_batch2_audit_20260722';
const PRODUCT_BACKUP_RELATION = `public.${PRODUCT_BACKUP_TABLE}`;
const AUDIT_RELATION = `public.${AUDIT_TABLE}`;
const EXPECTED_DECISION_CONTENT_SHA256 = 'c1c8c17f21fd5bd7be150f82aae70a8b3ceacf829bf592fdf78bbae34cc323f0';
...
function assertSnapshot(snapshot, label) {
  ...
  if (typeof snapshot.pricePerUnit !== 'string' || !/^\d+(?:\.\d+)?$/.test(snapshot.pricePerUnit)) {
    throw new Error(`${label}.pricePerUnit must be an exact non-negative decimal string`);
  }
...
function writePlan(filePath, decisions, decisionSha, applyPath, rollbackPath) {
  ...
  const lines = [
    '# Asia Deli Go catalog metadata batch 2',
    '',
    ...
    `- Evidence-insufficient holds: ${decisions.holds.length}.`,
