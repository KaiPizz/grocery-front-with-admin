...
import { validateDecisions } from '../scripts/catalog-metadata-batch6-plan.mjs';

const PROJECT_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const DECISIONS_PATH = 'docs/asiandeligo-catalog-metadata-batch6-decisions-20260722.json';
const PLAN_PATH = 'docs/asiandeligo-catalog-metadata-batch6-remediation-20260722.md';
const APPLY_PATH = 'docs/asiandeligo-catalog-metadata-batch6-apply-20260722.sql';
const ROLLBACK_PATH = 'docs/asiandeligo-catalog-metadata-batch6-rollback-20260722.sql';
const GENERATOR_PATH = path.join(PROJECT_ROOT, 'scripts/catalog-metadata-batch6-plan.mjs');
...
const EXPECTED_TRANSITION_SKUS = [
  'ADG-000556', 'ADG-000859', 'ADG-000526', 'ADG-000655', 'ADG-000656',
  'ADG-000901', 'ADG-000902', 'ADG-000943', 'ADG-001068', 'ADG-001120',
  'ADG-001121', 'ADG-001129', 'ADG-001130', 'ADG-001205',
];
const EXPECTED_HOLD_SKUS = [
  'ADG-000485', 'ADG-000658', 'ADG-000759', 'ADG-000804', 'ADG-000805',
  'ADG-000806', 'ADG-000807', 'ADG-000854', 'ADG-000389', 'ADG-000876',
  'ADG-000914',
];
const EXPECTED_TRANSITION_COUNT = EXPECTED_TRANSITION_SKUS.length;
const EXPECTED_PATCHES = {
  'ADG-000556': { storageZone: 'AMBIENT' },
  'ADG-000859': { allergens: ['sulphites'] },
  'ADG-000526': { storageZone: 'AMBIENT', countryOfOrigin: 'Dania' },
  'ADG-000655': { storageZone: 'AMBIENT' },
  'ADG-000656': { storageZone: 'AMBIENT' },
  'ADG-000901': { storageZone: 'AMBIENT' },
  'ADG-000902': { storageZone: 'AMBIENT' },
  'ADG-000943': { storageZone: 'AMBIENT' },
  'ADG-001068': { storageZone: 'AMBIENT' },
  'ADG-001120': { storageZone: 'AMBIENT' },
  'ADG-001121': { storageZone: 'AMBIENT' },
  'ADG-001129': { storageZone: 'AMBIENT' },
  'ADG-001130': { storageZone: 'AMBIENT' },
  'ADG-001205': { storageZone: 'AMBIENT' },
};
...
test('reviewed decisions lock the exact 14-transition and 11-hold cohort partition', () => {
  const decisions = validateDecisions(readDecisions());
  assert.equal(decisions.version, 1);
  assert.equal(decisions.batch, 'asiandeligo-catalog-metadata-batch6-20260722-v1');
...
test('generator emits tenant-pinned, exact-row, reversible metadata-only SQL', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'adg-metadata-batch6-sql-'));
  try {
    ...
    assert.match(applySql, /CREATE TABLE public\.asiandeligo_catalog_metadata_batch6_product_backup_20260722/);
    assert.match(applySql, /CREATE TABLE public\.asiandeligo_catalog_metadata_batch6_audit_20260722/);
    ...
    const backupInsert = applySql.indexOf('INSERT INTO public.asiandeligo_catalog_metadata_batch6_product_backup_20260722');
...
test('generator output is reproducible and matches all committed reviewed artifacts', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'adg-metadata-batch6-repro-'));
  try {
    const artifacts = generateArtifacts(directory);
...
test('validation rejects altered cohort, out-of-scope fields, and zero-change transitions', () => {
  ...
  const transition = partitionSwap.products.find((row) => row.sku === 'ADG-000556');
  const hold = partitionSwap.holds.find((row) => row.sku === 'ADG-000485');
  transition.sku = 'ADG-000485';
  hold.sku = 'ADG-000556';
  assert.throws(
  ...
  approvedValue.products.find((row) => row.sku === 'ADG-000556').target.storageZone = 'CHILLED';
  assert.throws(
  ...
  const storageOnly = invalidAllergen.products.find((row) => row.sku === 'ADG-000556');
  ...
  const missingNutrition = invalidNutrition.products.find((row) => row.sku === 'ADG-000526');
