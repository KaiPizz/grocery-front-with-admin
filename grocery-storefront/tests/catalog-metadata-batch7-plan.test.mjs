...
import { validateDecisions } from '../scripts/catalog-metadata-batch7-plan.mjs';

const PROJECT_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const DECISIONS_PATH = 'docs/asiandeligo-catalog-metadata-batch7-decisions-20260722.json';
const PLAN_PATH = 'docs/asiandeligo-catalog-metadata-batch7-remediation-20260722.md';
const APPLY_PATH = 'docs/asiandeligo-catalog-metadata-batch7-apply-20260722.sql';
const ROLLBACK_PATH = 'docs/asiandeligo-catalog-metadata-batch7-rollback-20260722.sql';
const GENERATOR_PATH = path.join(PROJECT_ROOT, 'scripts/catalog-metadata-batch7-plan.mjs');
...
const EXPECTED_TRANSITION_SKUS = [
  'ADG-001234', 'ADG-001387', 'ADG-001492', 'ADG-001493', 'ADG-000074',
  'ADG-000182', 'ADG-000217', 'ADG-000605', 'ADG-000793', 'ADG-000998',
  'ADG-001105', 'ADG-001166', 'ADG-001215', 'ADG-001571', 'ADG-000080',
  'ADG-000276', 'ADG-000110', 'ADG-000535', 'ADG-000265', 'ADG-000453',
  'ADG-000491',
];
const EXPECTED_HOLD_SKUS = [
  'ADG-001460', 'ADG-000606', 'ADG-000020', 'ADG-000035',
];
const EXPECTED_TRANSITION_COUNT = EXPECTED_TRANSITION_SKUS.length;
const EXPECTED_PATCHES = {
  'ADG-001234': { storageZone: 'AMBIENT' },
  'ADG-001387': { storageZone: 'AMBIENT' },
  'ADG-001492': { storageZone: 'AMBIENT' },
  'ADG-001493': { storageZone: 'AMBIENT' },
  'ADG-000074': { storageZone: 'AMBIENT' },
  'ADG-000182': { storageZone: 'AMBIENT' },
  'ADG-000217': { allergens: ['sulphites'], storageZone: 'AMBIENT' },
  'ADG-000605': { storageZone: 'AMBIENT' },
  'ADG-000793': { allergens: ['soybeans'], storageZone: 'AMBIENT' },
  'ADG-000998': { storageZone: 'AMBIENT' },
  'ADG-001105': { countryOfOrigin: 'Japonia' },
  'ADG-001166': { allergens: ['sulphites'], storageZone: 'AMBIENT' },
  'ADG-001215': { storageZone: 'AMBIENT' },
  'ADG-001571': { allergens: ['soybeans'], storageZone: 'AMBIENT' },
  'ADG-000080': { storageZone: 'AMBIENT' },
  'ADG-000276': { storageZone: 'AMBIENT' },
  'ADG-000110': { storageZone: 'AMBIENT' },
  'ADG-000535': { storageZone: 'AMBIENT' },
  'ADG-000265': { storageZone: 'AMBIENT' },
  'ADG-000453': { storageZone: 'AMBIENT' },
  'ADG-000491': { storageZone: 'AMBIENT' },
};
...
test('reviewed decisions lock the exact 21-transition and 4-hold cohort partition', () => {
  const decisions = validateDecisions(readDecisions());
  assert.equal(decisions.version, 1);
  assert.equal(decisions.batch, 'asiandeligo-catalog-metadata-batch7-20260722-v1');
...
test('generator emits tenant-pinned, exact-row, reversible metadata-only SQL', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'adg-metadata-batch7-sql-'));
  try {
    ...
    assert.match(applySql, /CREATE TABLE public\.asiandeligo_catalog_metadata_batch7_product_backup_20260722/);
    assert.match(applySql, /CREATE TABLE public\.asiandeligo_catalog_metadata_batch7_audit_20260722/);
    ...
    assert.match(applySql, /tgenabled = 'O'/);
    ...
    assert.match(rollbackSql, /tgenabled = 'O'/);
    ...
    const backupInsert = applySql.indexOf('INSERT INTO public.asiandeligo_catalog_metadata_batch7_product_backup_20260722');
...
test('generator output is reproducible and matches all committed reviewed artifacts', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'adg-metadata-batch7-repro-'));
  try {
    const artifacts = generateArtifacts(directory);
...
test('validation rejects altered cohort, out-of-scope fields, and zero-change transitions', () => {
  ...
  const transition = partitionSwap.products.find((row) => row.sku === 'ADG-001234');
  const hold = partitionSwap.holds.find((row) => row.sku === 'ADG-001460');
  transition.sku = 'ADG-001460';
  hold.sku = 'ADG-001234';
  assert.throws(
  ...
  approvedValue.products.find((row) => row.sku === 'ADG-001234').target.storageZone = 'CHILLED';
  assert.throws(
  ...
  const storageOnly = invalidAllergen.products.find((row) => row.sku === 'ADG-001234');
  ...
  const missingNutrition = invalidNutrition.products.find((row) => row.sku === 'ADG-001234');
