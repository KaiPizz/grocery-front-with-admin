...
import { validateDecisions } from '../scripts/catalog-metadata-batch8-plan.mjs';

const PROJECT_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const DECISIONS_PATH = 'docs/asiandeligo-catalog-metadata-batch8-decisions-20260722.json';
const PLAN_PATH = 'docs/asiandeligo-catalog-metadata-batch8-remediation-20260722.md';
const APPLY_PATH = 'docs/asiandeligo-catalog-metadata-batch8-apply-20260722.sql';
const ROLLBACK_PATH = 'docs/asiandeligo-catalog-metadata-batch8-rollback-20260722.sql';
const GENERATOR_PATH = path.join(PROJECT_ROOT, 'scripts/catalog-metadata-batch8-plan.mjs');
...
const EXPECTED_TRANSITION_SKUS = [
  'ADG-000492', 'ADG-000493', 'ADG-000537', 'ADG-000538', 'ADG-000539',
  'ADG-000564', 'ADG-000234', 'ADG-000352', 'ADG-000025', 'ADG-000069',
  'ADG-000089', 'ADG-000957', 'ADG-000308',
];
const EXPECTED_HOLD_SKUS = [
  'ADG-000105', 'ADG-000121', 'ADG-000702', 'ADG-000062', 'ADG-000179',
  'ADG-000382', 'ADG-000029', 'ADG-000034', 'ADG-000071', 'ADG-000349',
  'ADG-000350', 'ADG-000371',
];
const EXPECTED_TRANSITION_COUNT = EXPECTED_TRANSITION_SKUS.length;
const EXPECTED_PATCHES = {
  'ADG-000492': { storageZone: 'AMBIENT' },
  'ADG-000493': { storageZone: 'AMBIENT' },
  'ADG-000537': { storageZone: 'AMBIENT' },
  'ADG-000538': { storageZone: 'AMBIENT' },
  'ADG-000539': { storageZone: 'AMBIENT' },
  'ADG-000564': { storageZone: 'AMBIENT' },
  'ADG-000234': { storageZone: 'AMBIENT' },
  'ADG-000352': { storageZone: 'AMBIENT' },
  'ADG-000025': { storageZone: 'AMBIENT' },
  'ADG-000069': { storageZone: 'AMBIENT' },
  'ADG-000089': { storageZone: 'AMBIENT' },
  'ADG-000957': { storageZone: 'AMBIENT' },
  'ADG-000308': { storageZone: 'AMBIENT' },
};
...
test('reviewed decisions lock the exact 13-transition and 12-hold cohort partition', () => {
  const decisions = validateDecisions(readDecisions());
  assert.equal(decisions.version, 1);
  assert.equal(decisions.batch, 'asiandeligo-catalog-metadata-batch8-20260722-v1');
...
test('generator emits tenant-pinned, exact-row, reversible metadata-only SQL', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'adg-metadata-batch8-sql-'));
  try {
    ...
    assert.match(applySql, /CREATE TABLE public\.asiandeligo_catalog_metadata_batch8_product_backup_20260722/);
    assert.match(applySql, /CREATE TABLE public\.asiandeligo_catalog_metadata_batch8_audit_20260722/);
    ...
    const backupInsert = applySql.indexOf('INSERT INTO public.asiandeligo_catalog_metadata_batch8_product_backup_20260722');
...
test('generator output is reproducible and matches all committed reviewed artifacts', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'adg-metadata-batch8-repro-'));
  try {
    const artifacts = generateArtifacts(directory);
...
test('validation rejects altered cohort, out-of-scope fields, and zero-change transitions', () => {
  ...
  const transition = partitionSwap.products.find((row) => row.sku === 'ADG-000492');
  const hold = partitionSwap.holds.find((row) => row.sku === 'ADG-000105');
  transition.sku = 'ADG-000105';
  hold.sku = 'ADG-000492';
  assert.throws(
  ...
  approvedValue.products.find((row) => row.sku === 'ADG-000492').target.storageZone = 'CHILLED';
  assert.throws(
  ...
  const storageOnly = invalidAllergen.products.find((row) => row.sku === 'ADG-000492');
  ...
  const missingNutrition = invalidNutrition.products.find((row) => row.sku === 'ADG-000492');
