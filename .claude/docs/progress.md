...
| Product listing lacks category filter UI | Medium | 2026-05-14: `/categories`, `/categories/[slug]`, MobileBottomNav Categories, and the desktop category mega menu are live browse surfaces, but `/products` still has no explicit category filter control despite older docs claiming category filtering. 2026-05-14: Kimchi.pl category listing research saved in `.claude/docs/kimchi-category-listing-page-research.md`; next slice should merge category context with listing controls instead of leaving category pages as a plain grid. |
