...
| Product listing lacks category filter UI | Medium | 2026-05-14: `/categories`, `/categories/[slug]`, MobileBottomNav Categories, and the desktop category mega menu are live browse surfaces, but `/products` still has no explicit category filter control despite older docs claiming category filtering. Add this when finishing the broader B1 product-list integration, or keep category browsing intentionally separate and update the backlog acceptance criteria. |
