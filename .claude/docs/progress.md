...
## 2026-08-08 Asia Deli Go Pre-Launch QA Remediation (In Progress)

- Live desktop/mobile QA passed the core guest browse, search, product, cart,
  wishlist, and account-entry flows without console errors; shipment and
  payment integrations remain explicitly outside this audit.
- Cursor pagination controls (including the SSR-provided next cursor, distinct
  forward/backward page fixtures, and stale-response invalidation), Polish
  accessible action labels, partial/null/error catalog-filter handling, failed
  listing recovery, and mobile-menu dialog isolation now have test-first fixes
  on the pre-launch branch. Listing identity also includes channel/page size so
  old-tenant rows and cursors are suppressed immediately. Product meta/OG/Twitter
  descriptions are now
  normalized to at most 160 Unicode code points without truncating the visible
  product description or Product JSON-LD. Production is unchanged.
- Catalog audit proved that the filter metadata query receives only 100 of
  1,779 products. Known dietary, storage, and certification availability now
  comes from a coordinated tenant/category-scoped backend aggregate that scans
  the catalog once, while an incomplete or failed request cannot clamp prices
  or infer that other finite options are absent. Origin counts are suppressed
  whenever active filters are outside the count endpoint's scope. The backend
  and storefront candidates remain local and must ship backend-first.
- Owner/legal input remains required for the seller/controller's full legal
...
| ProductListingClient | ✅ | 2026-08-08: cursor-backed page controls, SSR cursor seeding, stale page-response invalidation, channel/page-size source isolation, one-query aggregate finite-option availability, partial/null/error fail-open status, failed-query recovery, and scoped origin-count truthfulness are covered by RED→GREEN Playwright regressions. 2026-05-15. Shared client listing shell for `/products` and `/categories/[slug]`: product query, category-scoped filter construction, sort, desktop filter panel, mobile draft/apply filter sheet, load-more, and responsive initial render for category no-JS coverage. 2026-05-24: all-products listings can now filter by discovered product categories while category pages keep their route-scoped category invariant. 2026-05-24: active committed filters are summarized outside the drawer/panel with removable chips so shoppers can see and recover from narrowed result sets. |
...
| Asia Deli Go homepage merchandising | ✅ | 2026-07-22: the hero carousel preserves every owner-enabled admin slide in its configured order instead of silently reducing the six-slide set to two per locale. Follow-up visual polish exposes compact edge arrows at every breakpoint and pins a tighter 4px indicator set near the banner bottom; mobile swipe, localized keyboard labels, and direct slide selection remain available. Verified with ESLint, pinned TypeScript 5.9.3, static config contracts (3/3), hero Playwright (6/6), release-parity Playwright (53/53), a production build, standalone smoke, and visual QA at 320px, 390px, and 1280px. Production remains unchanged pending the guarded release flow and fresh owner approval. 2026-07-21: groups nine configured categories into one compact browse section, moves the product shelf ahead of a single promotion, replaces the uncurated repeated-product campaign for this config with a truthful pickup guide, and suppresses known placeholder contact data. |
...
| Accessibility | 🔧 | 2026-08-08: localized Polish allergen/cart action labels and replaced the CSS-only mobile drawer with a modal dialog that isolates background landmarks, traps focus, has one close control, closes on Escape, and restores trigger focus. RED→GREEN Playwright covers these contracts; full site audit remains pending. ARIA labels on interactive elements, focus-visible ring, sr-only utility, landmark roles. 2026-05-10 mobile tap targets bumped to 44x44 (WCAG 2.5.5 AAA) on `MobileProductCard` + PD wishlist/add buttons. 2026-05-12: started page-by-page audit with homepage keyboard skip control to `main#main-content`, then audited Wishlist, Cart, and Checkout flows; saved/cart item actions now use product-specific accessible names, mobile cart checkout remains reachable without BottomNav, and checkout validation/choice/summary accordion states are covered by Playwright. |
| SEO meta tags | 🔧 | 2026-08-08: product meta, Open Graph, and Twitter descriptions now collapse whitespace and stop at a word-safe maximum of 160 Unicode code points while visible product copy and Product JSON-LD retain the full description. RED→GREEN Playwright covers long multilingual fallback copy. Overlong/duplicated product titles and missing English catalog translations remain content-data work. Admin SEO config page exists. 2026-05-12: storefront `generateMetadata()` now reads published config for title, description, canonical, OG image, and favicon; covered by Playwright RED→GREEN spec. |
...
|-------|----------|-------|
| Asia Deli Go legal pages need final seller/controller data | High | 2026-09-23: admin now has optional, backward-compatible legal-identity fields for owner-provided legal name, registration, identifiers, and addresses; terms/privacy/footer render them only after a real legal name is supplied and otherwise show an honest fallback. The owner's exact data and Polish consumer/privacy review are still required. Do not invent NIP/REGON/KRS, legal bases, retention, complaint/ADR, or food-withdrawal terms. |
...
| Asia Deli Go lacks a complete aggregate catalog-facet contract | Medium | 2026-08-08: a coordinated local backend/storefront candidate now resolves known dietary/storage/certification presence in one exact tenant/category-scoped catalog scan, replacing the capped 100-row inference and an abandoned 11-resolver prototype. Price extrema, allergen/category facets, and fully combined/search-scoped country counts still lack aggregates. The storefront therefore omits unproven price bounds, fails open on unknown/error metadata, and hides origin numbers whenever their scope would be misleading. |
