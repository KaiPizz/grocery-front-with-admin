...
- Cursor pagination controls, Polish accessible action labels, partial-catalog
  filter handling, and mobile-menu dialog isolation now have test-first fixes
  on the pre-launch branch. Product meta/OG/Twitter descriptions are now
...
- Catalog audit proved that the filter metadata query receives only 100 of
  1,779 products. Until the backend exposes aggregate facets, the storefront
  must not clamp prices or hide finite filters as if that page were exhaustive.
...
| ProductListingClient | ✅ | 2026-08-08: cursor-backed page controls, non-exhaustive facet fail-open behavior, and search count truthfulness are covered by RED→GREEN Playwright regressions. 2026-05-15. Shared client listing shell for `/products` and `/categories/[slug]`: product query, category-scoped filter construction, sort, desktop filter panel, mobile draft/apply filter sheet, load-more, and responsive initial render for category no-JS coverage. 2026-05-24: all-products listings can now filter by discovered product categories while category pages keep their route-scoped category invariant. 2026-05-24: active committed filters are summarized outside the drawer/panel with removable chips so shoppers can see and recover from narrowed result sets. |
