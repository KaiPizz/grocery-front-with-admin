...
- Cursor pagination controls, Polish accessible action labels, partial-catalog
  ...
  on the pre-launch branch. Bounded product SEO metadata remains in progress;
...
| SEO meta tags | 🐛 | 2026-08-08: production sampling found product titles/descriptions far beyond useful search-snippet bounds; bounded metadata fallback remediation is in progress. Admin SEO config page exists. 2026-05-12: storefront `generateMetadata()` now reads published config for title, description, canonical, OG image, and favicon; covered by Playwright RED→GREEN spec. |
